...
type Res<T, U> = IResult<T, U, VerboseError<T>>; // like IResult but takes a VerboseError instead.

/*
Generally the results in nom (IResults) are of the structure IResult<Input, Output, Error>
Where Input is the remaining input minus the parsed part, output is the parsed part, and error
is an error of parsing in case such one occurs
*/
...
fn n_to_m_digits<'a>(m: usize, n: usize) -> impl FnMut(&'a str) -> Res<&str, String> {
    move |input| {
        many_m_n(m, n, one_of("0123456789"))(input)
            .map(|(next_input, result)| (next_input, result.into_iter().collect()))
    }
}

fn ip_num(input: &str) -> Res<&str, u8> {
    // parse a single number in an IP (like 127 in 127.0.0.1)
...
mod tests {
    ...
    fn test_host() {
        assert_eq!(
            host("localhost:8080"),
            Ok((":8080", Host::HOST("localhost".to_owned())))
        );

        assert_eq!(
            host("example.org:8080"),
            Ok((":8080", Host::HOST("example.org".to_owned())))
        );

        assert_eq!(
            host("some-subsite.example.org:8080"),
            Ok((":8080", Host::HOST("some-subsite.example.org".to_owned())))
        );

        assert_eq!(
            host("example.123"),
            Ok((".123", Host::HOST("example".to_owned())))
        );
        assert_eq!(
            host("$$$.com"),
            Err(NomErr::Error(VerboseError {
    ...
    fn test_ipv4() {
        ...
        assert_eq!(
            ip("999.168.0.0:8080"), // will fail because of u8 restriction to value up to 255
            Err(NomErr::Error(VerboseError {
                errors: vec![
        ...
        // some debugging demonstration
        let e = match ip("999.168.0.0:8080").err().unwrap() {
            nom::Err::Error(e) => e,
            _ => VerboseError { errors: vec![] },
        };
        println!("{:?}", e);
        println!("{:?}", convert_error("999.168.0.0:8080", e));
    }
    ...
    fn test_uri() {
        assert_eq!(
            ...
            Ok((
                ...
                URI {
                    scheme: Scheme::HTTPS,
                    authority: None,
                    host: Host::HOST("www.zupzup.org".to_owned()),
                    port: None,
                    path: Some(vec!["about"]),
                    query: None,
                    fragment: None
        ...
        assert_eq!(
            ...
            Ok((
                ...
                URI {
                    scheme: Scheme::HTTP,
                    authority: None,
                    host: Host::HOST("localhost".to_owned()),
                    port: None,
                    path: None,
                    query: None,
                    fragment: None
        ...
        assert_eq!(
            uri("http://user:pwd@127.0.0.1:8080"),
            Ok((
                "",
                URI {
                    scheme: Scheme::HTTP,
                    authority: Some(("user", Some("pwd"))),
